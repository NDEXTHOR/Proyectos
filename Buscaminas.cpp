...
void Tablero::renderizar() {
    ...
    fo(i, 20) {
        ...
        fo(j, 20) {
            if (tabMarcadas[i][j] && tabUsuario[i][j] == '.')
                cout << "F "; // Flag
            else
                cout << tabUsuario[i][j] << " ";
        }
    ...
    fo(i, 20) {
        fo(j, 20) {
            if (tabPrograma[i][j] == false) {
                ...
                tabUsuario[i][j] =
                    '0' + minasAlrededor; // Se convierte a char, creo es por el ascii
            }
        }
    }
}
...
int Tablero::floodFill(int fil, int col) {
    if (tabUsuario[fil][col] != '.')
        return 0; // Caso basem, ya fue revelada

    int minasAlrededor = contarMinasAlrededor(fil, col), cont = 1;

    if (minasAlrededor == 0) {
        tabUsuario[fil][col] =
            ' '; // Si no hay minas al rededor la celda esta vacia, pero igual podria ser 0
        ...
        for (int i = fil - 1; i <= fil + 1; i++) {
            for (int j = col - 1; j <= col + 1; j++) {
                ...
                if (i >= 0 && i <= 19 && j >= 0 && j <= 19)
                    cont += floodFill(i, j); // Revisar rangos...
            }
        }
    } else {
...
int main() {
    ...
    while (true) {
        ...
        if (celdasSinMina == 360) { // Creo deben ser 40 minas entonces deberian 360 celdas sin
                                    // nada, por que 20x20 = 400
